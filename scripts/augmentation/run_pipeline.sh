...
#   2. package:  python bin/augmentation/build_manifests.py      (acoustic screen + manifests)
# ============================================================================
set -e
...
$PY bin/augmentation/seed_pool.py "$@"

echo "== stage 1a: rare / zero-coverage intents (${RARE_GEN_MODEL:-gemini-3.1-pro-preview}) =="
GEN_MODEL="${RARE_GEN_MODEL:-gemini-3.1-pro-preview}" $PY bin/augmentation/generate.py \
...
$PY bin/augmentation/generate.py

echo "== stage 2: deterministic validation =="
$PY bin/augmentation/validate.py

echo "== stage 3: multi-annotator judging panel (keep on 2-of-3 majority) =="
$PY bin/augmentation/judge.py ${JUDGE_ARGS:---models panel}
